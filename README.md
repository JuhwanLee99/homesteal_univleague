# Homsteal Univleague Web Project

대한민국 대학야구교류전 기반 리그인 Homsteal Univleague를 소개하고, 경기 데이터를 운영/조회하는 웹 플랫폼입니다.
더불어 **실시간 경기 기록(Scorekeeping) 및 중계용 오버레이 시스템**을 포함한 통합 야구 운영 기능을 제공합니다.

## ✨ 주요 기능 (Key Features)

리그 정보 제공 외에 다음 기능들을 제공합니다.
...
```bash
docker run --name homsteal-mysql -e MYSQL_ROOT_PASSWORD=homsteal -e MYSQL_DATABASE=homsteal \
...
```bash
export DATABASE_URL=mysql+pymysql://root:homsteal@localhost:3306/homsteal
```
...
```bash
mysql -uroot -phomsteal -h127.0.0.1 -e "SELECT * FROM matches" homsteal > matches.csv
mysql -uroot -phomsteal -h127.0.0.1 -e "SELECT * FROM teams" homsteal > teams.csv
mysql -uroot -phomsteal -h127.0.0.1 -e "SELECT * FROM players" homsteal > players.csv
mysql -uroot -phomsteal -h127.0.0.1 -e "SELECT * FROM batting_stats" homsteal > batting_stats.csv
mysql -uroot -phomsteal -h127.0.0.1 -e "SELECT * FROM pitching_stats" homsteal > pitching_stats.csv
```
...
* MySQL 연결 문자열 예시: `mysql+pymysql://user:password@host:3306/homsteal` (SQLAlchemy 사용)
...
* `/intro`: 리그 소개
* `/rules`: 리그 규정
* `/schedule`: 정규리그/포스트시즌 일정
* `/community`: 공지사항 + 자유게시판
* `/scoreboard`: 실시간 중계용 스코어보드
* `/scorekeeper`: 경기 기록 입력 시스템 (권한 필요)

© 2026 Homsteal Univleague.
